document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!whatsappButtons.length) {
    ...
    const idPrefix = `whatsapp-float-${Math.random().toString(36).slice(2, 10)}`;

    button.href = 'https://wa.me/31654216787';
    ...
    button.innerHTML = `
      <svg class="w-8 h-8" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 175.216 175.552">
        <defs>
          <linearGradient id="${idPrefix}-gradient" x1="85.915" x2="86.535" y1="32.567" y2="137.092" gradientUnits="userSpaceOnUse">
            <stop offset="0" stop-color="#57d163" />
            <stop offset="1" stop-color="#23b33a" />
          </linearGradient>
          <filter id="${idPrefix}-filter" width="1.115" height="1.114" x="-.057" y="-.057" color-interpolation-filters="sRGB">
        ...
        <path fill="url(#${idPrefix}-gradient)"
    ...
    whatsappButtons = [button];
  }

  const setVisible = (target) => {
    target.classList.remove('translate-x-[120%]', 'opacity-0');
  ...
  whatsappButtons.forEach((button) => {
    ...
    window.setTimeout(() => setVisible(button), delayMs);
  });
});
